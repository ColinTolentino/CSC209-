...
int setup_new_client(int fd, struct sockname *usernames) {
    char client_username[BUF_SIZE + 1];
    int user_index = 0;
    ...
    //We read right after we accept a connection
    int client_usernamePrompt = read(client_fd,client_username,BUF_SIZE); 
    if (client_usernamePrompt <= 0){
        return -1;
    }else{
        client_username[client_usernamePrompt] = '\0';
    }

    usernames[user_index].sock_fd = client_fd;
    usernames[user_index].username = malloc(sizeof(char)*client_usernamePrompt);
    strncpy(usernames[user_index].username, client_username, client_usernamePrompt-1);
    return client_fd;
}
...
int read_from(int client_index, struct sockname *usernames) {
    char *echo =  malloc(sizeof(char)*BUF_SIZE);
    char buf[BUF_SIZE + 1];

    //Edits the message that goes back to the client.
    int num_read = read(usernames[client_index].sock_fd, &buf, BUF_SIZE);
    buf[num_read] = '\0';
    //Add
    strcpy(echo,usernames[client_index].username);
    strcat(echo, ": ");
    strcat(echo, buf);
    ...
    for(int i = 0; i<MAX_CONNECTIONS; i++){
        ...
        if (num_read == 0 || write(fd, echo, strlen(echo)) != strlen(echo)) {
...
int main(void) {
    ...
    while (1) {
        ...
        // Is it the original socket? Create a new connection ... checks if the socket is active and if it is then set up a new connection
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
            if (usernames[index].sock_fd > -1 && FD_ISSET(usernames[index].sock_fd, &listen_fds)) {
                ...
                if (client_closed > 0) { //Gets into here if the client is finished
