...
int check_permissions(char *permissions, char *file_permissions){
    int count = 0;
    int check = 0; //If check becomes 1 then the element in permissions is in file_permissions.
    if(permissions[0] == 'd'){ //Checks if it is a directory
        return 0;
    }
    while(count < 9){
        for(int i = 0; i < 9; i++){
            if(permissions[count] == file_permissions[i]){
                check = 1;
            }
        }
        if(check == 1){ 
            count++;
            check = 0; //reset the check to 0 for the next element check.
        }
        else{
            return 0; 
        }
    }
    return 1; //Loop finishes meaning all elements in permissions are in file_permissions.
}
int check_permissions(char *permissions, char *file_permissions){
    int check; 
    if(permissions[0] == 'd'){ //Checks if it is a directory
    ...
    for(check = 0;check < 9; check++){
        if(permissions[check] != '-'){
            if(permissions[check] != file_permissions[check]){
                return 0;
            }
        }
    }
...
int main(int argc, char** argv) {
    char permission_requirements[11]; // 9 elements in the permissions
	char file_permissions[11]; // 9 elements in the permissions
    int size_of_file;
    int size_limit;
